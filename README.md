...
2. 希望借助 DHT 与各类 Tracker 的信息流，收集更多种子的元数据，用来搜索和整理自己需要的资料。但现有公开种子大多集中在色情类别，很难找到有价值的内容🐶。
3. Tracker 天然会记录客户端的 IP 和端口信息，具备一定的威胁情报价值。例如可以观察某个种子在哪些国家或地区最常被下载；在数据泄露安全事件中，能通过连接分布快速推测事件发生位置或影响范围。
